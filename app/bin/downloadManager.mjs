...
async function downloadUpdateGame(request) {
  ...
  try {
    ...
    const isDownloadRunning = runningDownloads.hasOwnProperty(`${request.gameId}`) && runningDownloads[request.gameId];
    if (error.code == 'EEXIST' && isDownloadRunning) {
      ...
      sendRequestReply(
        'nack',
        {
          newRequestId: runningDownloads[request.gameId],
        },
        request,
      );
      // And log a message
  ...
  switch (gameInfo.downloadType) {
    case 'steamcmd':
      try {
        // Make sure steamcmd itself is updated
        ...
        await sendRequestReply('status', replyText, request);

        // Pull our in-progress download from our tracking object
        runningDownloads[request.gameId] = false;
        ...
        if (error != null) {
          ...
          await sendRequestReply('error', error, request);
    ...
    default:
      ...
      await sendRequestReply('error', new Error('unsupported request'), request);
      setPingReply(moduleIdent, ipc, 'error');
      await unlockFile(`downloadManager-downloadUpdateGame-${request.gameId}`);
      return;
  }
